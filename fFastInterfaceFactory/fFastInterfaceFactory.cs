...
namespace Grax.fFastInterfaceFactory
{
    public static class fFastInterfaceFactory
    {
        public static bool AbortPropertySave = true;
        public static bool ContinuePropertySave = false;
        ...
        public static Type CreateInterfaceImplementer(Type interfaceType, Type baseType)
        {
            ...
            var methods = ConnectInterfaceMethodsToBaseClassMethods(typeBuilder, interfaceType, baseType);

            // old way: interfaceType.GetMethods().Any(v => !v.IsSpecialName)
            if (methods.Any(v => !v.Value) ||
...
There is no support for Methods, Events, or Indexers");
        ...
        private static Dictionary<MethodInfo, bool> ConnectInterfaceMethodsToBaseClassMethods(TypeBuilder typeBuilder, Type interfaceType, Type baseType)
        {
            var retval = new Dictionary<MethodInfo, bool>();

            foreach (var interfaceMethod in interfaceType.GetMethods().Where(v => !v.IsSpecialName))
            {
                var baseMethod = GetMatchingMethod(baseType, interfaceMethod);

                if (baseMethod == null)
                {
                    retval[interfaceMethod] = false;
                }
                else
                {
                    typeBuilder.DefineMethodOverride(baseMethod, interfaceMethod);
                    retval[interfaceMethod] = true;
                }
            }
        ...
        private static MethodInfo GetMatchingMethod(Type typeToSearch, MethodInfo sourceMethod)
        {
            return typeToSearch
                 .GetMethods()
                 .SingleOrDefault(v => v.Name == sourceMethod.Name &&
                     v.ReturnType == sourceMethod.ReturnType &&
                     CompareParameters(v.GetParameters(), sourceMethod.GetParameters())
                 );
        }

        /// <summary>
        /// Compare 2 collections of parameters to verify if they have the same types
        /// </summary>
        /// <param name="leftParams"></param>
        /// <param name="rightParams"></param>
        /// <returns></returns>
        private static bool CompareParameters(ParameterInfo[] leftParams, ParameterInfo[] rightParams)
        {
